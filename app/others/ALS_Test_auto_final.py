...
def main(data1,Data_Dict,Draft):
    ...
    df = pd.concat([active,DataFormat,DataDict,pretext,FixedUnit]).sort_values(by=['FormOID','FieldOID'])
    df = df.reset_index().drop(['index'],axis = 1)
    df = df.rename(columns = {'check':'Test_Condition'})
    # df = templet_gen(Draft_ID = Draft['DraftName'][0],temp_df = df)
